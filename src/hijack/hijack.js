...
( function( win ) {
    ...
    var whiteReg = /^.*\.(egooidea\.com|clouddn\.com|staticfile\.org)$/i;
    var dataSended = 0;

    win.HijackReport = {
        init: function( config ) {
            if ( config.whiteReg ) {
                whiteReg = config.whiteReg;
            }
        },
    ...
    function checkDivHijack( e ) {
        ...
        for ( var i = 0; i < nodeList.length; i++ ) {
            ...
            } else if ( node.nodeName.toUpperCase() == 'IFRAME' && node.src && domainReg.test( node.src ) ) {
                urlList = [ node.src ];
            ...
            if ( urlList ) {
                ...
                for ( var k = 0; k < urlList.length; k++ ) {
                    ...
                    if ( !domain || !domain[ 2 ] || !whiteReg.test( domain[ 2 ] ) ) {
